...
struct MessageWithFileData {
    ...
    std::string path; // Absolute path in our local filesystem
...
public:
    ...
    MessageWithFileData SendFile(std::string path);
    MessageWithFileData ModifyFile(std::string path); // Firstly, will delete file from the chat. And after this will send the file
}